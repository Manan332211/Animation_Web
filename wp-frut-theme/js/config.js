...
// Automatically adjust frame paths if running in a WordPress context
if (typeof window !== 'undefined' && window.wpTheme && window.wpTheme.themeUrl) {
  SITE_CONFIG.variants.forEach(variant => {
    if (!variant.framePath.startsWith('http') && !variant.framePath.startsWith('/')) {
      // Ensure there are no double slashes when prefixing the path
      const base = window.wpTheme.themeUrl.endsWith('/') ? window.wpTheme.themeUrl.slice(0, -1) : window.wpTheme.themeUrl;
      const path = variant.framePath.startsWith('/') ? variant.framePath : '/' + variant.framePath;
      variant.framePath = base + path;
    }
  });
}
